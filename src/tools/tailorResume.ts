...
// ─── All tailoring rules from memory ──────────────────────────────────────
...
Respond with ONLY the JSON object. No markdown fences.`,
  ...
  try {
    return JSON.parse(jsonMatch[0]) as TailoringResult;
  } catch {
    throw new Error("Failed to parse tailoring result JSON");
  }
}

export function saveTailoredLatex(
  tailoredLatex: string,
  outputDir: string,
  companyName: string
): string {
  // Format: Sachin_Durge_Frontend_Developer_Resume_CompanyName.tex
  const safeCompany = companyName
    .trim()
    .replace(/[^a-zA-Z0-9\s]/g, "")
    .replace(/\s+/g, "_");

  const filename = `Sachin_Durge_Frontend_Developer_Resume_${safeCompany}.tex`;
  const outputPath = path.join(outputDir, filename);

  fs.writeFileSync(outputPath, tailoredLatex, "utf-8");
  return outputPath;
}
